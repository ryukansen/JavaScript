...
```
Na Classe `Render`, é passado como parâmetro o arquivo visual do sistema (`template.phtml`), e o `array $args` será passado como parâmetro no método `render()`, as `keys` do `array $args` serão chamadas no `template.phtml` dentro de chaves `{name}` e `{email}`, e assim serão substituídas pelos valos passados no `array`.
